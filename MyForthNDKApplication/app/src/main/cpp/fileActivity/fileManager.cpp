...
namespace FileManager {
    ...
    void appendToFile(JNIEnv *env, const char *path, jstring content) {
        FILE *file = fopen(path, "a+");
        if (file) {
            fputs(FileManager::jstring2string(env, content), file);
            fclose(file);
            LOGI("Appended successfully");
        } else {
            LOGE("File non existent");
        }
    }
...
Java_com_example_myforthndkapplication_FileActivity_createFile(
        JNIEnv *env,
        jobject thiz,
        jstring dir,
        jstring data) {
    ...
    std::string complete =
            std::string(FileManager::jstring2string(env, dir)) + "/fileActivityTest.txt";

    LOGI("CompletePath: %s", complete.c_str());
    FileManager::saveToFile(env, complete.c_str(), data);

    return (*env).NewStringUTF("Hello from JNI (with file io)!");
}

const char *get_build_version(JNIEnv *env, jobject thiz) {
    jclass localClass = env->FindClass("com/example/myforthndkapplication/FileActivity");
...
JNIEXPORT jstring JNICALL
Java_com_example_myforthndkapplication_FileActivity_getCompiledAPI(JNIEnv *env, jobject thiz) {
    int date[6] = {0, 0, 0, 0, 0, 0};
    FileManager::get_date(date);
    std::string today = convertDate(date);
    const char *buildVersion = get_build_version(env, thiz);
    if (buildVersion) {
        const char *compiled = "Compiled with ABI " ABI ".\n";
...
JNIEXPORT jstring JNICALL
Java_com_example_myforthndkapplication_FileActivity_readFileText(
        JNIEnv *env,
        jobject thiz,
        jstring path) {

    std::string complete = std::string(FileManager::jstring2string(env, path)) + "/fileActivityTest.txt";
...
JNIEXPORT jstring JNICALL
Java_com_example_myforthndkapplication_FileActivity_appendToFile(
        JNIEnv *env,
        jobject thiz,
        jstring path,
        jstring data) {
    std::string complete =
            std::string(FileManager::jstring2string(env, path)) + "/fileActivityTest.txt";

    LOGI("CompletePath: %s", complete.c_str());
    int date[6] = {0, 0, 0, 0, 0, 0};
    FileManager::get_date(date);
    std::string today = convertDate(date);
    FileManager::appendToFile(env, complete.c_str(), data);

    return (*env).NewStringUTF("Hello from Append JNI!");
}
}